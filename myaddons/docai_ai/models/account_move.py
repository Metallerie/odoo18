...
class AccountMove(models.Model):
        ...
        for move in self:
            ...
            try:
                ...
                if vals:
                    ...
                    _logger.info(f"ℹ️ {label} {move.name or move.id} déjà avec JSON, pas de mise à jour")
    ...
    # -------------------------------------------------------------------------
    # CRON : Analyse des JSON déjà présents (compléter factures)
    # -------------------------------------------------------------------------
    @api.model
        ...
        try:
            ...
            for move in moves:
                try:
                    ...
                    move.action_docai_scan_json()  # méthode d'analyse déjà présente
    ...
    # -------------------------------------------------------------------------
    # Actions pour télécharger les JSON (boutons dans la vue)
    # -------------------------------------------------------------------------
    def action_docai_download_json_raw(self):
        """Ouvre le JSON brut (DocAI complet) dans un nouvel onglet"""
        self.ensure_one()
        return {
    ...
    def action_docai_download_json_min(self):
        """Ouvre le JSON simplifié (DocAI minimal) dans un nouvel onglet"""
        self.ensure_one()
        return {
            'type': 'ir.actions.act_url',
